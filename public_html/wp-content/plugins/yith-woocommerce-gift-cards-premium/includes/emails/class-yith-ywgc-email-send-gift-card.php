...
if ( ! class_exists( 'YITH_YWGC_Email_Send_Gift_Card' ) ) {
	...
	class YITH_YWGC_Email_Send_Gift_Card extends YITH_YWGC_Mail {
		...
		public function __construct() {
			...
			$this->heading = __( 'You got a gift card!', 'yith-woocommerce-gift-cards' );
			...
			// Fix for YITH Email Templates issue.
			if ( defined( 'YITH_WCET_PREMIUM' ) ) {
				add_filter( 'woocommerce_mail_callback', array( $this, 'filter_mail_callback' ), 9999 );
			}
		}
		 ...
		 * @return bool|void
		 */
		public function trigger( $object ) { // phpcs:ignore Universal.NamingConventions.NoReservedKeywordParameterNames.objectFound, Universal.NamingConventions.NoReservedKeywordParameterNames.caseFound
			$result = false;
		...
		public function init_form_fields() {
			$this->form_fields = array(
				...
				'ywgc_blind_carbon_copy'             => array(
					'title'                => __( 'Gift card admin notification', 'yith-woocommerce-gift-cards' ),
					'type'                 => 'yith_ywgc_field',
					'yith_ywgc_field_type' => 'onoff',
					'id'                   => 'ywgc_blind_carbon_copy',
					'description'          => __( 'If enabled, admin will receive a BCC email with the gift card code.', 'yith-woocommerce-gift-cards' ),
					'default'              => 'no',
				),
				'ywgc_blind_carbon_copy_to_buyer'    => array(
					'title'                => __( 'Gift card sender notification', 'yith-woocommerce-gift-cards' ),
					'type'                 => 'yith_ywgc_field',
					'yith_ywgc_field_type' => 'onoff',
					'id'                   => 'ywgc_blind_carbon_copy_to_buyer',
					'description'          => __( 'If enabled, the customer (the gift card sender) will receive a BCC email with the gift card code.', 'yith-woocommerce-gift-cards' ),
					'default'              => 'no',
				),
				'ywgc_blind_carbon_copy_additionals' => array(
					'title'                => __( 'BCC additional emails', 'yith-woocommerce-gift-cards' ),
					'type'                 => 'yith_ywgc_field',
					'yith_ywgc_field_type' => 'text',
					'id'                   => 'ywgc_blind_carbon_copy_additionals',
					'description'          => __( 'Create additional emails separating by a comma, to receive a BCC email with the gift card code.', 'yith-woocommerce-gift-cards' ),
					'default'              => '',
		...
		/**
		 * Filter WooCommerce mail callback as default if YITH Email Templates is enabled.
		 *
		 * @param callable $mail_callback The mail callback.
		 */
		public function filter_mail_callback( $mail_callback ) {
			return 'wp_mail';
		}
