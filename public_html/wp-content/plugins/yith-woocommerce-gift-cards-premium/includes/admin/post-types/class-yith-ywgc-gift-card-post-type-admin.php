...
if ( ! class_exists( 'YITH_YWGC_Gift_Cards_Post_Type_Admin' ) ) {
	...
	class YITH_YWGC_Gift_Cards_Post_Type_Admin extends YITH_Post_Type_Admin {
		...
		public function filter_views( $views ) {
			...
			$args_disabled = array(
				'post_type'   => $this->post_type,
				'post_status' => YITH_YWGC_Gift_Card::STATUS_DISABLED,
			);
			...
			$count_disabled     = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT( DISTINCT( ID ) ) FROM {$wpdb->posts} WHERE post_type= %s AND post_status= %s", 'gift_card', YITH_YWGC_Gift_Card::STATUS_DISABLED ) );
			...
			$disabled_url       = add_query_arg( $args_disabled, admin_url( 'edit.php' ) );
			$disabled_url_attrs = $this->is_disabled_view() ? 'class="current" aria-current="page"' : '';
			...
			$views = array(
				...
				'disabled'     => "<a href='{$disabled_url}' {$disabled_url_attrs}>" . esc_html__( 'Disabled', 'yith-woocommerce-gift-cards' ) . ' <span class="count">(' . esc_html( number_format_i18n( $count_disabled ) ) . ')</span></a>',
			);
		...
		/**
		 * Return true if this is the "disabled" view.
		 *
		 * @return bool
		 */
		private function is_disabled_view() {
			static $is_disabled = null;

			if ( is_null( $is_disabled ) ) {
				$is_disabled = isset( $_GET['post_status'] ) && YITH_YWGC_Gift_Card::STATUS_DISABLED === $_GET['post_status']; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			}
		...
		public function define_bulk_actions( $actions ) {
			...
			$actions['send_gift_cards'] = __( 'Send', 'yith-woocommerce-gift-cards' );
		...
		public function render_columns_custom( $column, $post_id ) {
			...
			switch ( $column ) {
				...
				case 'actions':
					$more_menu = array(
						'download-pdf' => array(
							'name' => __( 'Download PDF', 'yith-woocommerce-gift-cards' ),
							'url'  => $this->download_pdf_link( $post_id, $gift_card ),
						),
						'send-email'   => array(
		 ...
		 */
		public function download_pdf_link( $post_id, $gift_card ) {
			return wp_nonce_url(
				add_query_arg(
					array(
		...
		public function add_custom_dashboard_header() {
			...
			<script type="text/javascript">
				jQuery(function () {
					...
					jQuery('hr.wp-header-end').before("<a class='page-title-action create-code-custom-button'><?php echo esc_html__( 'Create Code', 'yith-woocommerce-gift-cards' ); ?></a>");
					jQuery('hr.wp-header-end').before("<a class='page-title-action export-import-custom-button'><?php echo esc_html__( 'Export/Import', 'yith-woocommerce-gift-cards' ); ?></a>");
