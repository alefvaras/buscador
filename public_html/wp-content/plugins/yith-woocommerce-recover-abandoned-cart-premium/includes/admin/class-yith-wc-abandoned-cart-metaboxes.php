	...
	class YITH_WC_RAC_Metaboxes {
		...
		public function show_cart_action_metabox( $post ) {
			...
			wc_get_template(
				...
				array(
					'cart_id'    => $post->ID,
					'email_sent' => ( 'no' === $email_sent || empty( $email_sent ) ) ? esc_html__( 'Not sent', 'yith-woocommerce-recover-abandoned-cart' ) : $email_sent,
				)
			);
		}
		...
		public function show_return_to_list() {
				printf( '<a href="%1$s" title="%2$s">%2$s</a>', esc_url( YITH_WC_Recover_Abandoned_Cart_Admin()->get_panel_page_uri( 'carts' ) ), esc_html__( 'Return to Abandoned Cart List', 'yith-woocommerce-recover-abandoned-cart' ) );
		}
	}
}
