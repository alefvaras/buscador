...
jQuery(function ($) {
	...
	$( '#ywpar_points_type_conversion input' ).on('click, change', function() {
		if ( 'override' !== $(this).val() ) {
			$('.ywpar_earn_points_conversion_rate .conversion-options').each( function() {
	...
	$( '#ywpar_affiliates_enabled' ).on('change', function() {
		if ( 'no' === $(this).val() ) {
			$(this).closest('.yith-plugin-fw__panel__section__content').each( function() {
				$(this).find('input').each( function() {
					$(this).attr( 'required',false);
				});
			});
