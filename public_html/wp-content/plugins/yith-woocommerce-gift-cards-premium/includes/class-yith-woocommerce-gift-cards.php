	...
	class YITH_WooCommerce_Gift_Cards {
		...
		public function create_gift_cards_category() {
			$labels = array(
				...
				'add_new_item'      => esc_html__( 'Save category', 'yith-woocommerce-gift-cards' ),
		...
		public function get_default_header_image() {
			...
			if ( '' === $default_header_image_url ) {
				$default_header_image_url = YITH_YWGC_ASSETS_IMAGES_URL . 'default-giftcard-main-image.jpg';
			}
			...
			if ( $obj instanceof YITH_YWGC_Gift_Card ) {
				if ( $obj->has_custom_design ) {
					switch ( $obj->design_type ) {
						...
						case 'default':
							$image            = wp_get_attachment_image_src( get_post_thumbnail_id( $obj->product_id ), apply_filters( 'ywgc_email_image_size', 'full' ) );
		...
		public function check_gift_card( $gift, $remove = false ) {
			...
			if ( ! $remove ) {
				$ywgc_minimal_cart_total = get_option( 'ywgc_minimal_cart_total_value' );

				if ( WC()->cart->total < $ywgc_minimal_cart_total ) {
					// translators: %s is the minimum cart total allowed to use the gift card.
					wc_add_notice( sprintf( __( 'In order to use the gift card, the minimum total amount in the cart has to be %s', 'yith-woocommerce-gift-cards' ), $ywgc_minimal_cart_total . get_woocommerce_currency_symbol() ), 'error' );
