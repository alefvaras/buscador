...
if ( ! function_exists( 'vi_wcpr_handle_sideload_unique' ) ) {
	/**
	 * @param $url
	 * @param $comment_id
	 * @param $post_id
	 ...
	 * @return int|string|WP_Error
	 */
	function vi_wcpr_handle_sideload_unique( $url, $comment_id, $post_id ) {
		global $wpdb;

		// Lấy filename từ URL
		$file_name = wp_basename( parse_url( $url, PHP_URL_PATH ) );

		// Check trong media library xem file này đã tồn tại chưa
		...
		if ( $attachment_id ) {
			return $attachment_id; // Nếu có rồi thì trả về luôn ID cũ
		}

		// Nếu chưa có thì tiến hành import với function bên trên
		return vi_wcpr_handle_sideload( $url, $comment_id, $post_id );
	}
}
