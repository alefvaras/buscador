...
class VI_WOOCOMMERCE_PHOTO_REVIEWS_Admin_Export {
	...
	public function export() {
		...
		if ( isset( $_POST['_woocommerce_photo_reviews_nonce'] ) && wp_verify_nonce( villatheme_sanitize_fields( $_POST['_woocommerce_photo_reviews_nonce'] ), '_woocommerce_photo_reviews_action_nonce' ) ) {
			...
			if ( count( $comments ) ) {
				$data_rows  = array();
				$header_row = apply_filters('viwcpr_export_header_row',array(
				...
				foreach ( $comments as $comment ) {
					...
					foreach ( $header_row as $item => $item_v ) {
						if ( in_array( $item, apply_filters('viwcpr_export_custom_fields',array(
							'rating',
							'reviews-images',
							'verified',
							'wcpr_custom_fields',
								...
								case 'reviews-images':
									if ( is_array( $meta ) && count( $meta ) ) {
										...
										foreach ( $meta as $reviews_image ) {
											if ( villatheme_is_url( $reviews_image ) ) {
