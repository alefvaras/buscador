...
if ( ! class_exists( 'YITH_YWGC_Frontend' ) ) {
	...
	class YITH_YWGC_Frontend {
		...
		protected function __construct() {
			...
			add_action( 'woocommerce_order_item_meta_start', array( $this, 'show_gift_card_code_on_order_item' ), 10, 2 );
			...
			add_filter( 'post_class', array( $this, 'add_sale_product_class' ), 10, 3 );
		}
		...
		public function show_gift_card_add_to_cart_button() {
			...
			if ( ! $product->is_purchasable() && apply_filters( 'yith_ywgc_show_gift_card_add_to_cart_button_is_purchasable', true ) ) {
				return;
			}
		 ...
		 */
		public function display_empty_gift_cards_table_state_view_customer( $args = array() ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter
			...
			if ( 'gift-card' === $product->get_type() ) {
				...
				if ( ! count( $amounts ) ) {
					...
					if ( $on_sale && $on_sale_value && apply_filters( 'ywgc_show_discounted_gift_card_product_price', true ) ) {
						...
						if ( $min_price_discounted === $max_price_discounted ) {
							$price_html = wc_price( $min_price_discounted );
						} else {
							$price_html = wc_format_price_range( $min_price_discounted, $max_price_discounted );
						}
		...
		/**
		 * Add 'sale' class if the gift card is on sale
		 *
		 * @param array $classes    Product classes.
		 * @param array $css_class  Additional class names added to the product.
		 * @param int   $product_id The product ID.
		 *
		 * @return array
		 * @since 4.0.0
		 */
		public function add_sale_product_class( $classes, $css_class, $product_id ) {
			$product = wc_get_product( $product_id );

			if ( ! ! $product && $product->is_type( 'gift-card' ) && $product->get_add_discount_settings_status() ) {
				$classes[] = 'sale';
			}
 ...
 * @since 2.0.0
 */
function YITH_YWGC_Frontend() { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName.FunctionNameInvalid, Universal.Files.SeparateFunctionsFromOO
