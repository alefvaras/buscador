	...
	class YITH_WC_Recover_Abandoned_Cart {
		...
		public function __construct() {
			...
			if ( empty( $cut_off_config ) ) {
				...
				switch ( $cut_off_type ) {
					case 'minutes':
						$this->cutoff = intval( $cut_off_time ) * MINUTE_IN_SECONDS;
						break;
					case 'hours':
						$this->cutoff = intval( $cut_off_time ) * HOUR_IN_SECONDS;
						break;
					case 'days':
						$this->cutoff = intval( $cut_off_time ) * DAY_IN_SECONDS;
						break;
				}
			}
			...
			if ( empty( $delete_cart_config ) ) {
				...
				$delete_cart_time = empty( $delete_cart_config['delete_cart_time'] ) ? 0 : $delete_cart_config['delete_cart_time'];
			...
			YITH_WC_Recover_Abandoned_Cart_Privacy();
		...
		public function enqueue_styles_scripts() {
			/**
			 * APPLY_FILTERS: yith_ywrac_include_frontend_scripts
			 *
		...
		public function add_post_type() {
			global $pagenow;
			$edit_item = __( 'View Abandoned Cart', 'yith-woocommerce-recover-abandoned-cart' );
			...
			if ( 'post.php' === $pagenow && isset( $_GET['post'] ) && isset( $_GET['action'] ) && 'edit' === $_GET['action'] ) {
				...
				$edit_item = sprintf( __( 'View %s\'s cart', 'yith-woocommerce-recover-abandoned-cart' ), get_the_title( $_GET['post'] ) );
			}

			$labels = array(
				'name'               => _x( 'Abandoned Carts', 'Post Type General Name', 'yith-woocommerce-recover-abandoned-cart' ),
		...
		public function ajax_grab_guest() {
			...
			ywrac_log( 'Grabbing guest cart for ' . $email, 'info' );
			if ( ! empty( $cart ) && $cart->post_type === $this->post_type_name ) {
				$post_id = $cart->ID;
				ywrac_log( 'updating cart from ajax_grab_guest dbg 1 ' . $cart->ID . ' - ' . $phone, 'info' );
				...
				if ( ! empty( $cart ) && $cart->post_type === $this->post_type_name ) {
					ywrac_log( 'updating cart from ajax_grab_guest dbg 2 ' . $cart->ID . ' - ' . $email . ' - ' . $phone, 'info' );
					$this->update_abandoned_cart(
						$cart->ID,
						$cart->post_date,
						array(
			...
			if ( $add_new ) {
				...
				ywrac_log( 'adding new abandoned cart (' . $post_id . ') - ' . $title . '(' . $meta_cart['user_email'] . ')', 'info' );
				ywrac_log( print_r( $meta_cart, true ), 'info' );
		...
		public function ajax_ywrac_delete_cart() {
			...
			if ( $posts ) {
				foreach ( $posts as $post ) {
					ywrac_log( 'deleting cart by Ajax (' . $_POST['cart_id'] . ')', 'info' );
		...
		public function add_abandoned_cart( $title, $metas ) {
			...
			/**
			 * APPLY_FILTERS: ywrac_add_abandoned_cart
			 *
			 * Add abandoned cart.
			 *
			 * @param bool $add
		...
		public function cart_updated() {
			...
			/**
			 * APPLY_FILTERS: ywrac_skip_register_cart
			 *
			 * Skip register grab.
			 *
			 * @param bool $to_skip
			...
			if ( is_user_logged_in() ) {
				...
				if ( $last_order ) {
					...
					/**
					 * APPLY_FILTERS: ywrac_wait_before_create_new_abandoned_cart
					 *
					 * Time to wait before to create a new abandoned cart.
					 *
					 * @param mixed $waiting_time default: HOUR_IN_SECONDS
					 * @param int $user_id
					 */
				...
				/**
				 * APPLY_FILTERS: ywrac_cart_updated_meta
				 *
				 * Abandoned cart updated metas.
				 *
				 * @param array $updated_metas
				 */
				$metas = apply_filters(
				...
				if ( ! $has_previous_cart && ! empty( $get_cart ) && ! $block_new_cart ) {
					...
					if ( ! empty( $get_cart ) && $this->get_subtotal_cart() > 0 ) {
						...
						if ( get_post_type( $post_id ) === $this->post_type_name ) {
							ywrac_log( 'Updating cart to open status from cart_updated ' . $post_id, 'info' );
							$this->update_abandoned_cart( $post_id, $post_date, array( 'cart_status' => 'open' ) );
						}
					} else {
						ywrac_log( 'Removing cart for current user (' . $user_id . ' - ' . $user_details->user_email . ') by cart_updated', 'info' );
				...
				if ( ! empty( $post ) && $post->post_type === $this->post_type_name ) {
					...
					ywrac_log( 'Updating cart to open status from cart_updated cookie ' . $post_id, 'info' );
					$this->update_abandoned_cart( $post_id, $post_date, array( 'cart_status' => 'open' ) );
				}
			}
		}
		...
		public function update_status( $cart ) {
			...
			ywrac_log( 'Update cart status for cart id ' . $cart->ID, 'info' );
			ywrac_log( ' - post modified: ' . $post_modified, 'info' );
			ywrac_log( ' - current time: ' . $current_time, 'info' );
			ywrac_log( ' - delta time: ' . $current_time - $post_modified, 'info' );
			ywrac_log( ' - cutoff: ' . $this->cutoff, 'info' );
			...
			if ( $delta > $this->cutoff ) {
				if ( 'open' === $current_status ) {
					...
					wp_update_post(
						array(
							'ID'        => $cart->ID,
							'post_date' => date( 'Y-m-d H:i:s', $current_time ), //phpcs:ignore
						)
					);
					...
					ywrac_log( '- Cart is set as Abandoned ' . $cart->ID, 'info' );
				}
			}

			ywrac_log( ' - delete_abandoned_time: ' . $this->delete_abandoned_time, 'info' );

			// delete the entry of cart if the deadline is over.
			if ( ! $this->delete_abandoned_time ) {
				if ( $delta > $this->delete_abandoned_time ) {
					if ( 'abandoned' === $current_status ) {
						wp_delete_post( $cart->ID, true );
						ywrac_log( '- deleting cart', 'info' );
					}
				}
			}
		}
		...
		public function remove_carts_abandoned_is_expired() {
			...
			ywrac_log( 'Checking expired carts from ' . date( 'Y-m-d H:i:s', $start_to_date ) . ' (deleted abandoned time = ' . $this->delete_abandoned_time . ')', 'info' );
			...
			ywrac_log( '- Found ' . print_r( $p, true ), 'info' ); // phpcs:ignore

			if ( ! empty( $p ) ) {
				foreach ( $p as $post ) {
					wp_delete_post( $post->ID, true );
					ywrac_log( ' - - Deleted expired cart with id ' . $post->ID, 'info' );
				}
			}
		}
		...
		public function remove_abandoned_cart_for_current_user( $user = null ) {
			...
			if ( isset( $_REQUEST['bulk_action'] ) && isset( $_REQUEST['action'] ) && strpos( $_REQUEST['action'], 'mark_' ) !== false ) {
				return;
			}
			$args = array(
				'post_type'   => $this->post_type_name,
			...
			$p = get_posts( $args );
			ywrac_log( 'Checking carts for current user ' . $user->data->user_email, 'info' );
			ywrac_log( '- args: ' . print_r( $args, true ), 'info' ); // phpcs:ignore
			ywrac_log( '- found: ' . print_r( $p, true ), 'info' ); // phpcs:ignore
			if ( ! empty( $p ) ) {
				foreach ( $p as $post ) {
					if ( metadata_exists( 'post', $post->ID, '_cart_status' ) ) {
						wp_delete_post( $post->ID, true );
						ywrac_log( 'Deleted cart for current user cart with id ' . $post->ID . ' user ' . get_post_meta( $post->ID, '_user_email', true ), 'info' );
					}
				}
			}
		}
		...
		public function get_cart_link( $cart_id, $email_id, $post_type = 'ywrac_cart' ) {
			...
			/**
			 * APPLY_FILTERS: ywrac_recover_cart_link
			 *
			 * Recover cart link.
			 *
			 * @param string $link
			 ...
			 * @param YITH_WC_Recover_Abandoned_Cart $_this
			 */
			return apply_filters( 'ywrac_recover_cart_link', $link, $cart_id, $email_id, $post_type, $this );
		}
		...
		public function register_pending_order_stats() {
			...
			$emails_sent = $order->get_meta( '_emails_sent' );
			$clicked     = false;
			if ( isset( $emails_sent[ $email_id ] ) ) {
				...
				$order->update_meta_data( '_emails_sent', $emails_sent );
				$order->update_meta_data( '_ywrac_email_id_processed', $email_id );
				$order->save();
			}
		...
		public function order_processed( $order_id ) {
			...
			if ( isset( $_COOKIE['ywrac_recovered_cart'] ) ) {
				...
				if ( 'yes' === get_option( 'ywrac_enable_email_admin' ) ) {
					/**
					 * DO_ACTION: send_recovered_cart_mail
					 *
					 * Send email to admin.
					 *
					 * @param array $args
			...
			ywrac_log( 'Removing cart for current user (' . $user_email . ') by order processed ' . $order_id, 'info' );
		 ...
		 * @param WC_Order|boolean $order The order instance.
		 */
		public function pending_order_counter( $order_id, $order = false ) {
		...
		public function register_order_pending_conversion( $order_id ) {
			...
			$order        = wc_get_order( $order_id );
			$is_recovered = $order->get_meta( '_ywrac_recovered' );
			$email_id     = $order->get_meta( '_ywrac_email_id_processed' );
			...
			$order->update_meta_data( '_cart_status', 'recovered' );
			$order->update_meta_data( '_ywrac_recovered', 'recovered' );
		...
		public function remove_cart_after_order_create( $order ) {
			...
			if ( ! $user ) {
				$user = $order->get_billing_email ( $order, '_billing_email' );
			} else {
				$user_info = get_user_by( 'id', $user );
				$user      = $user_info->user_email;
			}
			ywrac_log( 'Removing cart after order created (' . $user . ') by remove_cart_after_order_create', 'info' );
		...
		public function unsubscribe_from_mail() {
			...
			if ( empty( $customer_email ) || ! is_email( $customer_email ) ) {
				...
				ywrac_log( 'Removing cart of ' . $customer_email . ' by unsubscribe_from_email', 'info' );
		...
		public function declare_wc_features_support() {
			if ( class_exists( '\Automattic\WooCommerce\Utilities\FeaturesUtil' ) ) {
				\Automattic\WooCommerce\Utilities\FeaturesUtil::declare_compatibility( 'custom_order_tables', YITH_YWRAC_INIT, true );
			}
		}
