...
class WC_Pre_Orders_Admin_Products {
	...
	public function __construct() {
		...
		// Product list customizations bootstrap.
		add_action( 'current_screen', array( $this, 'maybe_boot_product_list_customizations' ) );
	}


	/**
	 * Load file and instantiate only on the Product list table.
	 *
	 * @since 2.3.0
	 * @param WP_Screen $screen
	 */
	public function maybe_boot_product_list_customizations( $screen ) {
		if ( ! $screen || 'edit-product' !== $screen->id ) {
			return;
		}

		if ( ! class_exists( 'WC_Pre_Orders_Admin_Product_List_Customizations', false ) ) {
			require_once 'class-wc-pre-orders-admin-product-list-customizations.php';
		}

		static $instance = null;
		if ( null === $instance ) {
			$instance = new WC_Pre_Orders_Admin_Product_List_Customizations();
		}
	}
	...
	public function save_product_tab_options( $post_id ) {
		...
		// Pre-order fee.
		$pre_order_fee = '';
		if ( isset( $_POST['_wc_pre_orders_fee'] ) ) {
			$pre_order_fee = wc_format_decimal( wc_clean( wp_unslash( $_POST['_wc_pre_orders_fee'] ) ) );
		}
		update_post_meta( $post_id, '_wc_pre_orders_fee', $pre_order_fee );
		 ...
		 */
		do_action( 'wc_pre_orders_save_product_options', $post_id, $is_enabled, $timestamp );

		if (
			isset( $_POST['_wc_pre_orders_availability_datetime'] )
			|| isset( $_POST['_wc_pre_orders_fee'] )
			|| isset( $_POST['_wc_pre_orders_when_to_charge'] )
		) {
			/**
			 ...
			 * @param int|false $timestamp  The availability date/time as a unix timestamp. False pre-orders have been disabled.
			 */
			do_action( 'wc_pre_orders_after_product_options_updated', $post_id, $is_enabled, $timestamp );
		}
	}
