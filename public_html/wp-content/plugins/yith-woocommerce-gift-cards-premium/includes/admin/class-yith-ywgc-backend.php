...
if ( ! class_exists( 'YITH_YWGC_Backend' ) ) {
	...
	class YITH_YWGC_Backend {
		...
		protected function __construct() {
			 ...
			 */
			add_filter( 'woocommerce_order_item_display_meta_value', array( $this, 'show_formatted_date' ), 10, 2 );
		 ...
		 * @since  1.0.0
		 */
		public function show_gift_card_code_on_order_item( $item_id, $item, $_product ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter
		...
		public function save_gift_card_data( $product_id ) {
			 ...
			 */
			$amounts = isset( $_POST['gift-card-amounts'] ) ? array_map( 'sanitize_text_field', wp_unslash( $_POST['gift-card-amounts'] ) ) : array(); // phpcs:ignore WordPress.Security.NonceVerification.Missing
			$amount  = isset( $_POST['gift_card-amount'] ) ? sanitize_text_field( wp_unslash( $_POST['gift_card-amount'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Missing
		...
		public function save_gift_card( $post_id, $post ) {
			...
			if ( ! isset( $_POST['product-type'] ) || ( YWGC_GIFT_CARD_PRODUCT_TYPE !== $_POST['product-type'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
				return;
			}
		 ...
		 * Custom condition
		 *
		 * @param bool     $cond  Condition to generate the gift card.
		...
		public function start_race_condition( $order_id ) {
			...
			$this->rc_active = $wpdb->query( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
				$wpdb->prepare(
					"UPDATE {$wpdb->postmeta} pm1, {$wpdb->postmeta} pm2
			...
			if ( $this->rc_active ) {
				$uniqid_result = $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
					$wpdb->prepare(
		...
		public function set_gift_card_category_to_product( $post_id ) {
			if ( isset( $_REQUEST['product-type'] ) && 'gift-card' !== $_REQUEST['product-type'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				return;
			}

			if ( ! isset( $_REQUEST['action'] ) && ! isset( $_REQUEST['action2'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				return;
			}

			if ( ( '-1' === $_REQUEST['action'] ) && ( '-1' === $_REQUEST['action2'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				return;
			}

			$selected_catergories            = isset( $_REQUEST['tax_input']['giftcard-category'] ) ? array_map( 'intval', $_REQUEST['tax_input']['giftcard-category'] ) : array(); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		public function set_gift_card_category_to_media() {
			...
			// Skip all request without a category set.
			if ( ! isset( $_REQUEST['categories1_id'] ) && ! isset( $_REQUEST['categories2_id'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				return;
			}
			...
			$action = '-1' !== $_REQUEST['action'] ? sanitize_text_field( wp_unslash( $_REQUEST['action'] ) ) : sanitize_text_field( wp_unslash( $_REQUEST['action2'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			if ( ! isset( $_REQUEST['media'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				return;
			}

			$media_ids = array_map( 'intval', $_REQUEST['media'] ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			$category_id = '-1' !== $_REQUEST['action'] ? intval( $_REQUEST['categories1_id'] ) : intval( $_REQUEST['categories2_id'] ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		public function update_taxonomy_count( $term_id, $taxonomy_name ) {
			...
			$count = (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM $wpdb->term_relationships, $wpdb->posts p1 WHERE p1.ID = $wpdb->term_relationships.object_id AND ( post_status = 'publish' OR ( post_status = 'inherit' AND (post_parent = 0 OR (post_parent > 0 AND ( SELECT post_status FROM $wpdb->posts WHERE ID = p1.post_parent ) = 'publish' ) ) ) ) AND post_type = 'attachment' AND term_taxonomy_id = %d", $term_id ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching

			$wpdb->update( $wpdb->term_taxonomy, compact( 'count' ), array( 'term_taxonomy_id' => $term_id ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		}
		...
		public function show_gift_cards_total_before_order_totals( $order_id ) {
			...
			foreach ( $order->get_items( 'yith_gift_card' ) as $gc_item ) {
				?>
				<tr>
					<td class="label"><?php echo esc_html__( 'Gift card: ', 'yith-woocommerce-gift-cards' ) . esc_html( $gc_item->get_code() ) . ''; ?></td>
					<td width="1%"></td>
					<td class="total"><?php echo wp_kses_post( wc_price( $gc_item->get_amount(), array( 'currency' => $currency ) ) ); ?></td>
				</tr>
				<?php
			}
		...
		public function filter_gift_card_page_query( $query ) {
			...
			if ( 'edit.php' === $pagenow && 'gift_card' === $post_type && isset( $_GET['balance'] ) && in_array( $_GET['balance'], array( 'used', 'active' ), true ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				if ( 'active' === $_GET['balance'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
					$meta_query = array(
						array(
		...
		public function show_as_string_order_item_meta_key( $display_key ) {
			if ( strpos( $display_key, 'ywgc' ) !== false ) {
				...
				} elseif ( '_ywgc_has_custom_design' === $display_key ) {
					$display_key = esc_html__( 'Has custom design', 'yith-woocommerce-gift-cards' );
				} elseif ( '_ywgc_delivery_notification_checkbox' === $display_key ) {
					$display_key = esc_html__( 'Delivery notification', 'yith-woocommerce-gift-cards' );
				}
			}
		 ...
		 * @return mixed
		 */
		public function show_formatted_date( $meta_value, $meta = '' ) {
			if ( $meta instanceof WC_Meta_Data && '_ywgc_delivery_date' === $meta->key ) {
		 ...
		 * @param string        $to_status   The next order status.
		...
		public function post_updated_messages( $messages ) {
			...
			$wresults    = $wpdb->get_results( $wpdb->prepare( $wtitlequery, $title, $post_id ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
 ...
 * @since 2.0.0
 */
function YITH_YWGC_Backend() { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName.FunctionNameInvalid, Universal.Files.SeparateFunctionsFromOO
