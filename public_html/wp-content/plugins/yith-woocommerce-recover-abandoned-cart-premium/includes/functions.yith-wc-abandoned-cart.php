...
if ( ! function_exists( 'yith_ywrac_create_sample_email_template_posts' ) ) {
	...
	function yith_ywrac_create_sample_email_template_posts() {
		...
		$cart_template_content = '<p style="text-align: center;">' . __( 'Hi', 'yith-woocommerce-recover-abandoned-cart') . ' {{ywrac.firstname}} {{ywrac.lastname}},</p>';
		...
		$cart_template_metas = array(
			'_ywrac_template_title'   => esc_html__( 'There is something in your cart!', 'yith-woocommerce-recover-abandoned-cart' ),
			'_ywrac_template_content' => $cart_template_content,
		...
		$pending_order_template_content = '<p style="text-align: center;">' . __( 'Hi', 'yith-woocommerce-recover-abandoned-cart') . ' {{ywrac.firstname}} {{ywrac.lastname}},</p>';
		...
		$pending_order_template_metas = array(
			'_ywrac_template_title'   => esc_html__( 'Complete your order with 10% off', 'yith-woocommerce-recover-abandoned-cart' ),
			'_ywrac_template_content' => $pending_order_template_content,
			'_ywrac_email_active'     => 'no',
...
if ( ! function_exists( 'yith_ywrac_locate_template' ) ) {
	...
	function yith_ywrac_locate_template( $path, $var = null ) {
		...
		/**
		 * APPLY_FILTERS: yith_ywrac_locate_template
		 *
		 * Locate template.
		 *
...
if ( ! function_exists( 'yith_ywrac_get_excerpt' ) ) {
	...
	function yith_ywrac_get_excerpt( $post_id ) {
		...
		$post_content = get_post_meta( $post_id, '_ywrac_template_content', true );
		$post_content = empty( $post_content ) ? $post->post_content : $post_content;
		/**
		 * APPLY_FILTERS: yith_ywrac_get_content_num_words
		 *
		 * Get content number of words.
		 *
		 * @param int $num_words default: 10
		 */
		$num_of_words = apply_filters( 'yith_ywrac_get_content_num_words', 10 );
		return wp_trim_words( $post_content, $num_of_words );
	}
}
...
if ( ! function_exists( 'ywrac_is_customer_unsubscribed' ) ) {
	...
	function ywrac_is_customer_unsubscribed( $user = null ) {
		...
		/**
		 * APPLY_FILTERS: ywrac_allow_current_user
		 *
		 * Allow current user.
		 *
		 * @param bool $allow
		 ...
		 */
		if ( apply_filters( 'ywrac_allow_current_user', false, $customer_id ) ) {
			return false;
		}
...
if ( ! function_exists( 'ywrac_get_cron_interval' ) ) {
	...
	function ywrac_get_cron_interval() {
		...
		/**
		 * APPLY_FILTERS: ywrac_cron_interval
		 *
		 * Cron Interval.
		 *
...
if ( ! function_exists( 'ywrac_new_metas_for_email_templates' ) ) {
	/**
	 * Update some new metas for the email template cpt
	 *
	 * @return void
	 */
	function ywrac_new_metas_for_email_templates() {
		if ( 'no' === get_option( 'ywrac_email_templates_updated', 'no' ) ) {
			$args = array(
				'numberposts' => -1,
			...
			foreach ( $templates as $i => $template ) {
				update_post_meta( $template->ID, '_ywrac_template_title', $template->post_title );
				update_post_meta( $template->ID, '_ywrac_template_content', $template->post_content );
			}
			update_option( 'ywrac_email_templates_updated', 'yes' );
		}
	}
}

if ( ! function_exists( 'ywrac_log' ) ) {
	/**
	 * Write LOG
	 *
	 ...
	 * @return void
	 */
	function ywrac_log( $message, $type ) {
		/**
		 * APPLY_FILTERS: ywrac_enable_log
		 *
		 * Enable plugin log.
		 *
		 * @param bool $active default = false.
		 */
		if ( apply_filters( 'ywrac_enable_log', false ) ) {
			YITH_RAC_Logger::log( $message, $type ); // phpcs:ignore
		}
	}
}
