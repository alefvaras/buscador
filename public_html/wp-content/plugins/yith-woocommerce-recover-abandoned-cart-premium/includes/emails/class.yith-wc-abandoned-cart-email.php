...
if ( ! class_exists( 'YITH_YWRAC_Send_Email' ) ) {
	...
	class YITH_YWRAC_Send_Email extends WC_Email {
		...
		public function trigger( $args ) {
			...
			$this->type          = $args['type'];
			$this->cart_id       = isset( $args['cart_id'] ) ? $args['cart_id'] : 0;
			/**
			 * APPLY_FILTERS: ywrac_email_template
			 *
			 * Provide the cart email html template
			 *
			 * @param string $path template path.
			 * @param WC_Email $wc_mail Wc email class.
		...
		public function get_from_address( $from_email = '' ) {
			$from_email = apply_filters( 'woocommerce_email_from_address', get_option( 'ywrac_email_sender' ), $this, $from_email );
		...
		public function get_from_name( $from_name = '' ) {
			$from_name = apply_filters( 'woocommerce_email_from_name', get_option( 'ywrac_sender_name' ), $this, $from_name );
