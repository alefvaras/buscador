...
if ( ! class_exists( 'YITH_YWGC_Emails' ) ) {
	...
	class YITH_YWGC_Emails {
		...
		public function __construct() {
			...
			add_action( 'ywgc_gift_card_email_after_preview', array( $this, 'show_expiration_date' ), 15 );
		 ...
		 */
		public function show_expiration_date( $gift_card ) {
			$default_gift_product = wc_get_product( get_option( YWGC_PRODUCT_PLACEHOLDER ) );

			if ( is_object( $gift_card ) && is_object( $default_gift_product ) && $gift_card->product_id === $default_gift_product->get_id() ) {
				return;
			}
			...
			echo '<div class="ywgc-expiration-message">' . wp_kses_post( $expiration_message ) . '</div>';
		}
		...
		public function show_introductory_text( $text, $gift_card ) {
			 ...
			 */
			?>
			<p class="center-email"><?php echo wp_kses_post( nl2br( apply_filters( 'ywgc_gift_cards_email_before_preview_text', $text, $gift_card ) ) ); ?></p>
			<?php
		}
 ...
 * @since 2.0.0
 */
function YITH_YWGC_Emails() { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName.FunctionNameInvalid, Universal.Files.SeparateFunctionsFromOO
