(function($){

    'use strict';
    $('#ywrac-mailslog-content #doaction.button').on( 'click', function(){
        window.onbeforeunload = '';
    });

    $('#ywrac_user_guest_enabled-privacy').on( 'change', function (){
        if (  $(this).is(':checked')) {
            $('tr.yith-plugin-fw-panel-wc-row.textarea').show();
        } else {
            $('tr.yith-plugin-fw-panel-wc-row.textarea').hide();
        }
    });

    $('#ywrac_user_guest_enabled-ever').on( 'change', function (){
        if (  $(this).is(':checked')) {
            $('tr.yith-plugin-fw-panel-wc-row.textarea').hide();
        }
    });

    $('#ywrac_user_guest_enabled-never').on( 'change', function (){
        if (  $(this).is(':checked')) {
            $('tr.yith-plugin-fw-panel-wc-row.textarea').hide();
        }
    });

   $('#ywrac_user_guest_enabled-privacy').change();

    /****
     * Send email
     */


    var send_email_btn = $('.ywrac_send_email'),
    send_email_func = function() {
		$(document).on( 'click', '.yith-plugin-fw__action-button--sendmail-action .yith-plugin-fw__action-button__menu__item', function( e ) {
			e.stopPropagation();
			e.preventDefault();
			var $t = $(this),
				$select_template = $t.data('id'),
			    cart_id = $t.data('cart_id'),
				type = $t.data('type'),
				ajax_loader    = ( typeof yith_ywrac_admin !== 'undefined' ) ? yith_ywrac_admin.block_loader : false;

			$t.append( '<img src="'+ajax_loader+'" >' );
			$.post(yith_ywrac_admin.ajaxurl, {
                action : 'ywrac_email_send',
                cart_id: cart_id,
                email_template: $select_template,
                type: type,
                security:  yith_ywrac_admin.send_email_nonce
            }, function (resp) {

                if( resp.email_sent !='no' ){
                    var label = yith_ywrac_admin.sent_label + ' <i>"' + resp.email_name + '"</i><br>' + yith_ywrac_admin.sent_label_on + ' ' + resp.email_sent;
                    if ($t.parents('.yith-ywrac-info-cart').length > 0) {
                        $t.parents('.yith-ywrac-info-cart').find('.ywrac_email_status').html(label);
                    } else {
                        $('.email_status[data-id="' + cart_id + '"]').html(label);
                    }
                    $t.find('img').remove();
                }
            });
		});
        send_email_btn.on('click', function (e) {
            e.stopPropagation();
			e.preventDefault();
            var $t = $(this),
                $select_template = $t.prev('select'),
                ajax_loader    = ( typeof yith_ywrac_admin !== 'undefined' ) ? yith_ywrac_admin.block_loader : false,
                type = $t.data('type'),
                cart_id = $t.data('id');

            if( $select_template.length == 0){
                $select_template = $('#ywrac-email-template');
            }
			$t.append( '<img src="'+ajax_loader+'" >' );

            $.post(yith_ywrac_admin.ajaxurl, {
                action : 'ywrac_email_send',
                cart_id: cart_id,
                email_template: $select_template.val(),
                type: type,
                security:  yith_ywrac_admin.send_email_nonce
            }, function (resp) {

                if( resp.email_sent !='no' ){
                    var label = yith_ywrac_admin.sent_label + ' ' + resp.email_name + ' (' + resp.email_sent + ')';
                    if ($t.parents('.yith-ywrac-info-cart').length > 0) {
                        $t.parents('.yith-ywrac-info-cart').find('.ywrac_email_status').html(label);
                    } else {
                        $('.email_status[data-id="' + cart_id + '"]').html(label);
                    }
                    $t.find('img').remove();
                }
            });
        });
    };

    send_email_func();


    /****
     * Send email test
     */

    var send_email_test_btn = $('.ywrac-button-sent-email'),
        send_email_test_func = function() {
            send_email_test_btn.on('click', function (e) {
                e.preventDefault();
                e.stopPropagation();
                var $t = $(this),
                    $select_template = $t.data('id'),
                    ajax_loader    = ( typeof yith_ywrac_admin !== 'undefined' ) ? yith_ywrac_admin.block_loader : false,
                    email_to_sent = $('#_ywrac_email_to_send').val();

                $t.after( ' <img src="'+ajax_loader+'" >' );
                $('.email-sent-label').remove();

                $.post(yith_ywrac_admin.ajaxurl, {
                    action : 'ywrac_email_test_send',
                    email_to_sent: email_to_sent,
                    email_template: $select_template,
                    security:  yith_ywrac_admin.send_email_nonce
                }, function (resp) {
                    if(resp.email_sent == 1){
                        $t.next().remove();
                        send_email_test_btn.after(' <span class="email-sent-label">'+yith_ywrac_admin.sent_label_test +'</span>');
                    }

                });
            });
        };

    send_email_test_func();

    /**
     * Reset Reports
     */
    var reset_reports_btn = $('.ywrac-reset-reports'),
        reset_reports = function() {
            reset_reports_btn.on( 'click', function( e )  {
                e.stopPropagation();
                var ajax_loader    = ( typeof yith_ywrac_admin !== 'undefined' ) ? yith_ywrac_admin.block_loader : false,
                    nonce = $('#ywrac_reset_reports_nonce').val();
                var confirm = window.confirm( yith_ywrac_admin.reset_confirmation );
                if ( confirm === true ) {
                    $(this).after( ' <img src="'+ajax_loader+'" >' );

                    $.post(yith_ywrac_admin.ajaxurl, {
                        action : 'ywrac_reset_reports',
                        nonce: nonce,
                    }, function (resp) {
                        if( resp.reset === true ){
                            $(this).next().remove();
                            location.reload();
                        }

                    });

                }
            } )
    };
    reset_reports();


    /**
     * Register toggle enabled
     */
    $(document).on('change', '.ywrac-toggle-enabled input', function () {
        var enabled = $(this).val() === 'yes' ? 'yes' : 'no',
            container = $(this).closest('.ywrac-toggle-enabled'),
            email_template_id = container.data('email-template-id'),
            security = container.data('security');

        $.ajax({
            type: 'POST',
            data: {
                action: 'email_template_toggle_enabled',
                id: email_template_id,
                enabled: enabled,
                security: security
            },
            url: yith_ywrac_admin.ajaxurl,
            success: function (response) {
                if (typeof response.error !== 'undefined') {
                    alert(response.error);
                }
                window.onbeforeunload = function () {
                    return null;
                }
            }

        });
    });
	$('#yith_woocommerce_recover_abandoned_cart_email').find('.yith-add-button').detach().insertAfter('#yith_woocommerce_recover_abandoned_cart_email .yith-plugin-fw__panel__content__page__title');

	/* Edit Email Template js */
	$('#_ywrac_template_title').keyup( function() {
		$('.yith-plugin-ui--ywrac_email-post_type #post-body-content #title').val($(this).val());
		$('#_ywrac_template_title').css('border','none');
	})

	$('.yith-plugin-ui--ywrac_email-post_type #ywrac-post-type__save').on( 'click', (e) => {
		e.preventDefault();
		if ( $('#_ywrac_template_title').val() === '' ) {
			$('#_ywrac_template_title').css('border','1px solid red');
			$('html, body').animate({
				scrollTop: $("#_ywrac_template_title").offset().top - 40
			}, 1000);
		} else {
			let frm = $('.yith-plugin-ui--ywrac_email-post_type form#post');
			frm.append('<input type="hidden" name="post_status" value="publish" />');
			frm.submit();
		}
	});

})(jQuery);