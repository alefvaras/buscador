                        ...
                        <div class="woosb-title">
                            ...
                            if ( $product->is_type( 'variable' ) ) {
                                if ( ( WPCleverWoosb_Helper()->get_setting( 'variations_selector', 'default' ) === 'woovr' ) && class_exists( 'WPClever_Woovr' ) ) {
                                    ...
                                    $table_layout         = apply_filters( 'woosb_variations_table_layout', false );
                                    ...
                                    if ( $table_layout ) {
                                        ...
                                        $tr_tag = $th_tag = $td_tag = 'div';
                                    }

                                    if ( is_array( $attributes ) && ( count( $attributes ) > 0 ) ) {
                                        echo '<form class="variations_form woosb_variations_form" action="' . esc_url( $product->get_permalink() ) . '" data-product_id="' . esc_attr( absint( $product->get_id() ) ) . '" data-product_variations="' . esc_attr( $variations_attr ) . '">';
                                        echo( $table_layout ? '<table class="variations" cellspacing="0" role="presentation"><tbody>' : '<div class="variations">' );
                                        ...
                                        foreach ( $attributes as $attribute_name => $options ) {
                                            ...
                                            echo '<' . $tr_tag . ' class="' . esc_attr( 'variation variation-' . $attribute_name_sz ) . '">';
                                            echo '<' . $th_tag . ' class="label"><label for="' . esc_attr( $attribute_name_sz ) . '">' . wc_attribute_label( $attribute_name ) . '</label></' . $th_tag . '>';
                                            echo '<' . $td_tag . ' class="value">';
                                            wc_dropdown_variation_attribute_options( apply_filters( 'woosb_variation_attribute_options', [
                                                    'options'          => $options,
                                                    'attribute'        => $attribute_name,
                                                    'selected'         => count( $options ) === 1 ? $options[0] : false,
                                                    'product'          => $product,
                                                    'show_option_none' => sprintf( /* translators: attribute name */ WPCleverWoosb_Helper()->localization( 'choose', esc_html__( 'Choose %s', 'woo-product-bundle' ) ), esc_html( wp_strip_all_tags( wc_attribute_label( $attribute_name ) ) ) )
                                            ], $item ) );
                                            echo '</' . $td_tag . '>';
                                            echo '</' . $tr_tag . '>';
                                        }

                                        echo( $table_layout ? '</tbody></table><!-- /.variations -->' : '</div><!-- /.variations -->' );
