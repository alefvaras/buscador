...
class WC_Pre_Orders_Manager {
	 ...
	 * Gets all pre-orders
	 *
	 * Eager loads all orders and can exhaust memory on large stores.
	 * Prefer use of `WC_Pre_Orders_Data_Store::get_pre_orders()` which yield
	 * a lazy `Generator` instead.
	 *
	 * @since 1.0
	 * @deprecated 2.3.0 use {@see WC_Pre_Orders_Data_Store::get_pre_orders()}
	 *
	 * @return WC_Order[] Array of order objects.
	 */
	public static function get_all_pre_orders(): array {
		// One-off hydratation for legacy callers.
		return iterator_to_array(
			WC_Pre_Orders_Data_Store::get_pre_orders(),
			false // reindex keys 0..n, keeps memory a bit lower
		);
	}
		...
		if ( WC_Pre_Orders::is_hpos_enabled() ) {
			$order_ids = $wpdb->get_results(
				$wpdb->prepare(
					...
					LEFT JOIN {$wpdb->prefix}wc_orders_meta AS status_meta ON items.order_id = status_meta.order_id
					WHERE
						...
						order_meta.meta_value = '1' AND
						status_meta.meta_key = '_wc_pre_orders_status' AND
						status_meta.meta_value IN ('active', 'completed', 'cancelled')
					",
					$product->get_id()
				)
			);
			...
			$order_ids = $wpdb->get_results(
				$wpdb->prepare(
					...
					LEFT JOIN {$wpdb->postmeta} AS status_meta ON items.order_id = status_meta.post_id
					WHERE
						items.order_item_type = 'line_item' AND
						item_meta.meta_key = '_product_id' AND
						item_meta.meta_value = %s AND
						post_meta.meta_key = '_wc_pre_orders_is_pre_order' AND
						post_meta.meta_value = '1' AND
						status_meta.meta_key = '_wc_pre_orders_status' AND
						status_meta.meta_value IN ('active', 'completed', 'cancelled')
					",
					$product->get_id()
	...
	public static function get_users_pre_orders( $user_id = null, $page_number = 1, &$query = null ) {
		...
		if ( WC_Pre_Orders::is_hpos_enabled() ) {
			$args    = array(
				...
				'meta_query'  => array( // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_query
					...
					array(
						...
						'value'   => array( 'active', 'completed', 'cancelled' ),
						'compare' => 'IN',
					),
				),
				'type'        => 'shop_order',
			...
			$wp_args = array(
				...
				'fields'         => 'ids',
				'posts_per_page' => wc_get_theme_support( 'pagination' ) ? get_option( 'posts_per_page' ) : 20,
	...
	public static function change_release_date_for_all_pre_orders( $product, $new_availability_date, $message = '' ) {
		...
		$is_enabled = ! $is_past_date && $timestamp && WC_Pre_Orders_Product::product_can_be_pre_ordered( $product_id );

		/** This hook is documented in includes/admin/class-wc-pre-orders-admin-products.php */
		do_action( 'wc_pre_orders_after_product_options_updated', $product_id, $is_enabled, $timestamp );
	...
	public static function complete_pre_order( $order, $message = '' ) {
		...
		add_filter( 'wc_pre_orders_processing_order_completion', '__return_true' );
		...
		remove_filter( 'wc_pre_orders_processing_order_completion', '__return_true' );
	}
