...
<table class="shop_table cart" id="yith-ywrac-table-list" cellspacing="0" style="width: 100%;">
	...
	foreach ( $cart_content['cart'] as $key => $cart_item ) :
		...
		if ( isset( $cart_item['ywdpd_discounts'] ) ) {
			$discounts     = current( $cart_item['ywdpd_discounts'] );
			$dynamic_price = $discounts['current_price'];
		}
		if ( ! is_null( WC()->cart ) ) {
			if ( WC()->cart->display_prices_including_tax() ) {
				$product_price = wc_get_price_including_tax( $_product, array( 'price' => $dynamic_price ) );
			} else {
				$product_price = wc_get_price_excluding_tax( $_product, array( 'price' => $dynamic_price ) );
			}
		} else {
			$product_price = wc_get_price_to_display( $_product, array( 'price' => $dynamic_price ) );
		}
