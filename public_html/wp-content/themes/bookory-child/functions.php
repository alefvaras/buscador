...
function bookory_child_improved_search() {
    ...
    // Normalizar: quitar espacios, guiones y convertir a minúsculas
    $normalized_keyword = strtolower(preg_replace('/[\s\-\_\.]+/', '', $search_keyword));
    
    $suggestions = [];
    
    // Query SQL para búsqueda flexible
    $sql = "
    ...
    $sql .= "
        ...
        AND (
            LOWER(REPLACE(REPLACE(REPLACE(REPLACE(p.post_title, ' ', ''), '-', ''), '_', ''), '.', '')) LIKE %s
    ...
    // Ordenar por relevancia: primero los que empiezan con el término
    $sql .= " ORDER BY 
        CASE 
            WHEN LOWER(REPLACE(REPLACE(p.post_title, ' ', ''), '-', '')) LIKE %s THEN 1
