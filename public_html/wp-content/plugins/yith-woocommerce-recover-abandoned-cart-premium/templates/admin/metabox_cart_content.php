...
?>
<style>
	.post-type-ywrac_cart .yith-plugin-fw__panel__header-nav { display: none !important;}
</style>
<table class="shop_table cart" id="yith-ywrac-table-list" cellspacing="0">
	...
	<tr>
		...
		<th class="product-single"><?php esc_html_e( 'Price', 'yith-woocommerce-recover-abandoned-cart' ); ?></th>
		<th class="product-quantity"><?php esc_html_e( 'Qty', 'yith-woocommerce-recover-abandoned-cart' ); ?></th>
	...
	foreach ( $cart_content['cart'] as $cart_item_key => $cart_item ) :
		...
		if ( 'incl' == $tax_display_cart ) {
			...
			//$tax_incl_html  = '<br><small class="woocommerce-Price-taxLabel tax_label">' . WC()->countries->inc_tax_or_vat() . '</small>';
			$price_subtotal = wc_price( $price, array( 'currency' => $currency ) );
			$price_total    = wc_price( $price * $qty, array( 'currency' => $currency ) );
		} else {
	...
	if ( $tax_total ) :
		if ( $tax_display_cart == 'excl' ) :
			?>
			<tr>
				<td scope="col" colspan="3" style="text-align: right"><strong><?php echo esc_html( WC()->countries->tax_or_vat() ); ?></strong></td>
	...
	if ( $total ) :
		...
		?>
		<tr>
			<td scope="col" colspan="3" style="text-align: right"><strong><?php esc_html_e( 'Cart Total', 'yith-woocommerce-recover-abandoned-cart' ); ?></strong></td>
