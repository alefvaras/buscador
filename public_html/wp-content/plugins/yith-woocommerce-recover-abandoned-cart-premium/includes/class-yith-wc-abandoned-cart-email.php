	...
	class YITH_WC_Recover_Abandoned_Cart_Email {
		...
		public function add_metabox() {
			if ( ywrac_check_valid_admin_page( $this->post_type_name ) ) {
				...
				/* Remove the save box */
				remove_meta_box( 'submitdiv', 'ywrac_email', 'side' );
			}
		}
		...
		public function get_email_templates( $type = 'all', $only_active = false ) {
			...
			/**
			 * APPLY_FILTERS: ywrac_get_email_templates
			 *
				...
				foreach ( $emails as $email ) {
					...
					if ( ! empty( $carts ) ) {
						...
						foreach ( $carts as $cart ) {
							$cart_id = ( 'order' === $type ) ? $cart->get_id() : $cart->ID;
							// check if the emails was sent for the cart from 1.1.3.
							if ( metadata_exists( 'post', $cart_id, '_user_id' ) ) {
								$current_user_id = get_post_meta( $cart_id, '_user_id', true );
								$current_user_id = ( 0 == $current_user_id || empty( $current_user_id ) ) ? get_post_meta( $cart_id, '_user_email', true) : $current_user_id ;
								if ( in_array( $current_user_id, $user_ids ) ) { //phpcs:ignore
									if ( 'order' !== $type ) {
										wp_delete_post( $cart_id );
										ywrac_log( 'Removed cart from email_cron with id ' . $cart_id, 'info' );
									}
									continue;
								} else {
							...
							$already_sent = false;
							if ( is_array( $emails_sent ) && count( $emails_sent ) ) {
								foreach ( $emails_sent as $key => $email_sent ) {
									$_key = $sitepress ? yit_wpml_object_id( $key, 'post', true, $sitepress->get_default_language() ) : $key;
							...
							if ( $already_sent || ( is_array( $email_sent ) && in_array( $cart_id, $email_sent ) ) ) { //phpcs:ignore
								continue;
							}
							...
							if ( 'cart' === $type ) {
								...
								$lang          = $cart->get_meta( 'wpml_language', true );
								...
								/**
								 * APPLY_FILTERS: ywrac_get_start_to_date_for_order
								 *
								 * Get order start date.
								 *
								 * @param string $start_to_date
								 * @param float|int $cutoff
								 */
								$start_to_date = apply_filters( 'ywrac_get_start_to_date_for_order', $start_to_date, $cutoff );
		...
		public function email_send_test( $email_to_send, $email, $lang ) {
			...
			$template_content  = get_post_meta( $email->ID, '_ywrac_template_content', true );
			$template_content  = empty( $template_content ) ? $email->post_content : $template_content;
			...
			/**
			 * APPLY_FILTERS: ywrac_recover_button_label
			 *
			 * Recover button label.
			 *
			 * @param $string $recover_button_label
			 * @param WP_Post $email email template cpt
			 */
			...
			/**
			 * APPLY_FILTERS: ywrac_test_template_content
			 *
			...
			/**
			 * DO_ACTION: send_rac_mail
			 *
			 * Send test email.
			 *
			 * @param array $args list of metas
			 */
		...
		public function email_send( $cart_id, $email, $lang, $type = 'cart' ) {
			...
			if ( 'cart' === $type || empty( $type ) ) {
				...
				/**
				 * APPLY_FILTERS: ywrac_check_cart_before_send_email
				 *
				 * Check before to send email.
				 *
				 * @param bool $send
				 * @param mixed $cart_content_meta
				 */
				...
				if ( ! $order || 'pending' !== $order->get_status() || 'yes' === $order->get_meta( 'is_a_renew' ) ) {
					return false;
				}
				...
				/**
				 * APPLY_FILTERS: ywrac_cart_link_args
				 *
				 * Cart link arguments.
				 *
				 * @param array $args
			...
			/**
			 * APPLY_FILTERS: ywrac_skip_user_from_mail
			 *
			 * Skip user while email sending.
			 *
			 * @param boolean $to_send
			...
			$template_content  = get_post_meta(  $email_id, '_ywrac_template_content', true );
			$template_content  = empty( $template_content ) ? $email->post_content : $template_content;
			$template_content  = nl2br( $template_content );
			...
			$result = array(
				'email_sent' => substr( $email_sent, 0, -3 ),
				'email_name' => $email->post_title,
			);
		...
		public function create_coupon( $cart_id, $email_id ) {
			...
			/**
			 * APPLY_FILTERS: ywrac_coupon_code_length
			 *
			 * Coupon code length.
			 *
			...
			/**
			 * APPLY_FILTERS: ywrac_get_coupon_code
			 *
			 * Coupon code.
			 *
			...
			/**
			 * APPLY_FILTERS: ywrac_coupon_args
			 *
			 * Coupon creation arguments.
			 *
			 * @param array $args
			 * @param int $email_id
			 */
