...
class WC_Pre_Orders {
	...
	/**
	 * @var WC_Pre_Orders_Compat_Subscriptions
	 */
	...
	public function __construct( $base_file ) {
		...
		// Clear transients when orders are modified
		add_action( 'woocommerce_new_order', array( $this, 'clear_pre_order_transients' ), 10, 0 );
		add_action( 'woocommerce_order_status_changed', array( $this, 'clear_pre_order_transients' ), 10, 0 );
		add_action( 'woocommerce_checkout_order_processed', array( $this, 'clear_pre_order_transients' ), 10, 0 );
		add_action( 'woocommerce_order_deleted', array( $this, 'clear_pre_order_transients' ), 10, 0 );
		add_action( 'woocommerce_order_updated', array( $this, 'clear_pre_order_transients' ), 10, 0 );
		add_action( 'woocommerce_order_refunded', array( $this, 'clear_pre_order_transients' ), 10, 0 );
		add_action( 'wc_pre_order_status_changed', array( $this, 'clear_pre_order_transients' ), 10, 0 );
		add_action( 'save_post_product', array( $this, 'clear_pre_order_transients' ), 10, 0 ); // Availability date update
		add_action( 'woocommerce_process_product_meta', array( $this, 'clear_pre_order_transients' ), 10, 0 ); // Availability date update
		add_action( 'update_product_meta', array( $this, 'clear_pre_order_transients' ), 10, 0 ); // Availability date update
	}
	...
	public function load_classes() {
		if( is_admin() ){
			...
			include_once WC_PRE_ORDERS_PLUGIN_PATH . '/includes/class-wc-pre-orders-privacy.php';
		}

		// Load data store class
		require 'class-wc-pre-orders-data-store.php';
		...
		// Load subscriptions compatibility
		require 'compat/class-wc-pre-orders-compat-subscriptions.php';
		$this->subscriptions = new WC_Pre_Orders_Compat_Subscriptions();
	...
	/**
	 * Clear pre-order related transients when orders are modified.
	 *
	 * This ensures dropdowns and counts are fresh when orders change.
	 *
	 * @since 2.3.0
	 */
	public function clear_pre_order_transients() {
